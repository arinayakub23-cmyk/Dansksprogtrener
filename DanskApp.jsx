  ...
  return (
    <div className="max-w-md mx-auto p-4 font-sans bg-gray-50 min-h-screen">
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {Object.keys(topics).map(k => (
          <button key={k} onClick={() => {setCurT(k); setCurC(0); setFeedback(null);}} 
            className={`px-4 py-2 rounded-full border whitespace-nowrap transition-all ${curT === k ? 'bg-blue-500 text-white shadow-md' : 'bg-white text-gray-700 border-gray-300'}`}>
            {topics[k].label}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
        ...
        <textarea 
          className="w-full h-24 p-4 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-blue-400 transition-all"
          value={inputs[key] || ""} 
        ...
        {feedback && (
          <div className={`mt-4 p-4 rounded-xl border ${feedback.score === 'load' ? 'bg-gray-100 border-gray-200' : 'bg-green-50 border-green-200 text-green-800'}`}>
